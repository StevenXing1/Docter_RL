...
class DocterEnv(gym.Env):
    ...
    def init(self, normalize, display, **kwargs):
        ...
        if normalize:
            self.gameOb = PLE(
                self.game,
                fps=30,
                state_preprocessor=self.get_ob_normalize,
            ...
            self.gameOb = PLE(
                self.game,
                fps=30,
                state_preprocessor=self.get_ob,
                display_screen=display,
            )
        ...
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=(len(self.game.getGameState()),), dtype=np.float32
        )
        self.gameOb.init()
    ...
    def step(self, action):
        ...
        truncated = False  # No time limit truncation in base env
        info = {}
        return (self.gameOb.getGameState(), reward, terminated, truncated, info)

    def reset(self, seed=None, options=None):
        if seed is not None:
            self.seed(seed)
        self.gameOb.reset_game()
        obs = self.gameOb.getGameState()
        info = {}
        return obs, info
