...
def plot_evaluation_results(results_path, save_path=None):
    ...
    ax.set_title(
        f"Reward Distribution (μ={np.mean(rewards):.2f}, σ={np.std(rewards):.2f})"
    )
    ax.grid(True, alpha=0.3)
    ...
    ax.axhline(
        np.mean(steps), color="r", linestyle="--", label=f"Mean: {np.mean(steps):.1f}"
    )
    ax.set_xlabel("Episode")
    ...
    ax = axes[1, 1]
    bp = ax.boxplot(
        [rewards, steps], labels=["Rewards", "Steps"], patch_artist=True, showmeans=True
    )
...
def compare_models(model_paths, labels=None, save_path=None):
    ...
    for scores, label in zip(all_scores, labels):
        if len(scores) > 0:
            ...
            moving_avg = np.convolve(scores, np.ones(window) / window, mode="valid")
            ax.plot(
                range(window - 1, len(scores)), moving_avg, label=label, linewidth=2
            )
    ax.set_xlabel("Episode")
...
def main():
    ...
    parser.add_argument(
        "--labels", type=str, nargs="+", help="Labels for comparison mode"
    )
    parser.add_argument("--output", type=str, help="Output path for saving plot")
    parser.add_argument(
        "--window", type=int, default=100, help="Window size for moving average"
    )
