...
def load_checkpoint_info(checkpoint_path):
    ...
    try:
        ...
        if 'scores' in checkpoint:
            ...
            if len(scores) > 0:
                info['final_mean_reward'] = np.mean(scores[-100:]) if len(scores) >= 100 else np.mean(scores)
                info['final_std_reward'] = np.std(scores[-100:]) if len(scores) >= 100 else np.std(scores)
        ...
        if 'config' in checkpoint:
            ...
            info['learning_rate'] = config.get('training', {}).get('learning_rate', 'N/A')
...
def compare_models(model_paths, output_dir=None):
    ...
    if 'final_mean_reward' in df.columns:
        ...
        for idx, row in ranked.iterrows():
            ...
            print(f"   Mean Reward: {row['final_mean_reward']:.2f} ± {row['final_std_reward']:.2f}")
...
def main():
    ...
    parser.add_argument('--output', '-o', type=str, default='results/comparison',
